...
/* Default config file */
  ...
  if (env) {
    ...
    if (env) {
      ...
      fprintf(stderr,
              "Warning: Unable to locate configuration directory, default "
              "config not loaded.\n");
    }
  }
  ...
  if (pub_or_sub == CLIENT_SUB) {
    if (cfg->clean_session == false && (cfg->id_prefix || !cfg->id)) {
      if (!cfg->quiet)
        fprintf(stderr,
                "Error: You must provide a client id if you are using the -c "
                "option.\n");
      return 1;
    }
  ...
  for (i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "-p") || !strcmp(argv[i], "--port")) {
      ...
      if (i == argc - 1) {
        ...
        if (pub_or_sub == CLIENT_PUB) {
          ...
          if (mosquitto_sub_topic_check(argv[i + 1]) == MOSQ_ERR_INVAL) {
            fprintf(stderr,
                    "Error: Invalid subscription topic '%s', are all '+' and "
                    "'#' wildcards correct?\n",
                    argv[i + 1]);
            return 1;
          }
      ...
      if (i == argc - 1) {
        ...
        if (mosquitto_sub_topic_check(argv[i + 1]) == MOSQ_ERR_INVAL) {
          fprintf(stderr,
                  "Error: Invalid filter topic '%s', are all '+' and '#' "
                  "wildcards correct?\n",
                  argv[i + 1]);
          return 1;
        }
        ...
        cfg->filter_outs =
            realloc(cfg->filter_outs, cfg->filter_out_count * sizeof(char *));
...
#ifdef WITH_SOCKS
  if (cfg->socks5_host) {
    rc = mosquitto_socks5_set(mosq, cfg->socks5_host, cfg->socks5_port,
                              cfg->socks5_username, cfg->socks5_password);
    if (rc) {
      mosquitto_lib_cleanup();
      return rc;
    }
...
#ifdef WITH_SRV
  if (cfg->use_srv) {
    rc = mosquitto_connect_srv(mosq, cfg->host, cfg->keepalive,
                               cfg->bind_address);
  } else {
...
#else
        FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, errno, 0, (LPTSTR)&err,
                      1024, NULL);
#endif
